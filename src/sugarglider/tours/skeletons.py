"""Pure deterministic start-on-perimeter ellipse skeleton construction."""
...
from sugarglider.tours.models import DirectionPreference, TourDirection
...
def routing_points_with_sampled_hard_anchors(
    ...
    entries.extend(
        (progress(point), 1, index, point) for index, point in enumerate(hard_points)
    )
    entries.sort(key=lambda value: (value[0], value[1], value[2]))
    return (start, *(entry[3] for entry in entries), start)
