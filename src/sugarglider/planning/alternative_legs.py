...
class LowOverlapBeamSearch:
    """Assemble gateway-routed alternatives into globally analyzed paths."""
    ...
    async def assemble(
        ...
        *,
        ...
        if closed and not _same_coordinate(routing_points[0], routing_points[-1]):
            assembled_points = (*routing_points, routing_points[0])
        if not closed and _same_coordinate(assembled_points[0], assembled_points[-1]):
