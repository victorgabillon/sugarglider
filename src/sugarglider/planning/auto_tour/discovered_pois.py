...
from sugarglider.planning.auto_tour.approaches import approach_candidates_for_feature
...
from sugarglider.planning.auto_tour.ranking import marginal_utility, poi_reward
...
def shortlist_route_pois(
    *,
    ...
    already = tuple(
        ...
        if visit.visit_distance_m <= visit.arrival_tolerance_m
    )
    ...
    for match in matches:
        ...
        if len(routing_points) < 2:
            continue
        radius = (
            match.approach.arrival_tolerance_m if match.approach is not None else None
        )
        if radius is None:
            continue
...
def build_poi_visits(
    *,
    ...
    visits: list[DiscoveredPoiVisit] = []
    ...
    for match in sorted(matches, key=_match_key):
        ...
        if approach is None:
            continue
        radius = approach.arrival_tolerance_m
        if match.distance_m > radius:
            continue
...
def estimate_insertion(
    *,
    ...
    approach = match.approach
    if approach is None:
        raise ValueError("POI insertion requires a meaningful approach")
